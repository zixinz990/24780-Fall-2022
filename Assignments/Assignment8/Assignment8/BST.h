...
class TreeNode {
   public:
    T theItem;  // T is the type of item stored (could be string, float, class
                // object, etc.)
...
class BST {
   protected:
    int nodeCount =
        0;  // "manually" maintained by insertItem and deleteItem functions
    TN* theRoot;
   ...
   public:
    ...
    // if a node that matches the given item is found, delete the node from the
    // tree returns true if the item was found and deleted, returns false if not
    // found (a bit complex, so we'll write together in lecture and then you can
    // just use Nestor's)
    bool deleteItem(const T& anItem, TN* parentNode = nullptr,
                    TN* currNode = nullptr);
   ...
   public:
    ...
    // If item is not already in BST, place it at the correct location and
    // increment the overall nodeCount of the tree
    void insertItem(const T& newItem, TN* startNode = nullptr);

    // Returns node that matches given item. Returns nullptr if item is not in
    // tree Optionally starts the search at a node that is not the root,
    // allowing for recursive searching (although search can be done without
    // recursion, ignoring second parameter)
    TN* findItem(const T& anItem, TN* startNode = nullptr);

    // Finds the item and returns the item count. Makes use of findItem()
    // function Returns zero if the item is not in the BST
    int retrieveItemCount(const T& anItem);

    // Tranverses the BST and prints out (to a string stream) each of the items
    // (on its own line) in the BST in correct order, along with the item's
    // count (can use recursion) typical output:
    //			 here {8}
    //           justice {2}
    ...
    if (theRoot != nullptr) {
        ...
        if (currNode == nullptr)
            return false;  // item not found. possible exit from function
                 ...
                 currNode->theItem) {  // found the node, time to delete
            ...
            // if only left child, promote left child of current from grandchild
            // of parent to child of parent node
            ...
            // if only right child, promote right child of current from
            // grandchild of parent to child of parent node
            ...
            // if there are two children (neither leftChild nor rightChild is
            // nullptr)
            else {
                ...
                // recursive call, but node to delete is guaranteed to have 0 or
                // 1 child
        ...
        // keep looking in left or right sub-tree, with current as parent
            ...
            return deleteItem(anItem, currNode,
                              currNode->leftChild);  // recursive call
        else
            return deleteItem(anItem, currNode,
                              currNode->rightChild);  // recursive call
    }
...
inline void BST<T, TN>::insertItem(const T& newItem, TN* currNode) {
    ...
    if (theRoot == nullptr) {
        ...
        // 	2) Start with the root as the current node if current node is
        // nullptr
        if (currNode == nullptr) {
            currNode = theRoot;
        }
        ...
        // 	a) If current node does not have left child, make new node the
        // left child of current node and DONE 	b) If current node has a left
        // child, make the left child the current node and return to Step 3
        // (recursion)
        ...
        // 	a) If current node does not have right child, make new node the
        // right child of current node and DONE 	b) If current node has a
        // right child, make the right child the current node and return to Step
        // 3 (recursion)
        else {
